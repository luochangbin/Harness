...
## 必须行为
- 执行 `executor_support.py inspect`（decision = use，selected = opencode）
- **不询问**执行器选择
- 直接走 OpenCode 路径（首次：生成 session ID → set-session → snapshot → Worker）
...
## 通过判定
PASS = 零询问、config.yaml 字节不变、Worker 以 OpenCode 调用；FAIL = 任何执行器询问或配置改写。
