...
## 仓库状态
已初始化 AR 仓库；active AR `AR-007-verify-loop`（phase: verify）；`.ar.yaml` 含 `verify_result: fail`、`verify_failures: 3`；verification.md 记录了前三次失败（均回到 build 修复后复验又失败）；tasks.md 全部勾选。
