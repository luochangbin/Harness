...
def build_fixture(root, change="codespec-001-test"):
    """构造可归档仓库：codespec/.codespec/config.yaml + SPEC.md + DESIGN.md + changes/<change>/。"""
    _write(os.path.join(root, "codespec", "changes", change, "tasks.md"), TASK_EVIDENCE)
    _write(os.path.join(root, "codespec", ".codespec", "config.yaml"), u'''# CodeSpec 工作流项目配置
language: zh-CN
modules:
  - id: auth
    label: 认证
...
其他设计内容。
''')
    _write(os.path.join(root, "codespec", "changes", change, ".codespec.yaml"),
...
- **THEN** 发送重置链接
''')
    _write(os.path.join(root, "codespec", "changes", change, "design.md"), u'''# codespec-001-test 设计
...
class ArchiveChangeTest(unittest.TestCase):
    ...
    def _rewrite_state(self, text):
        _write(os.path.join(self.change_dir(), ".codespec.yaml"), text)
    ...
    def test_plan_rejects_non_archive_phase(self):
        self._rewrite_state(u"codespec: {}\n".format(self.change) + u'''tier: full
phase: build
modules: [auth]
verify_result: pass
archive_confirmation: confirmed
...
''')
    ...
    def test_plan_rejects_path_escape(self):
        before = all_file_hashes(self.root)
        for evil in ("../evil", "codespec-001-test/../x", ".."):
...
''')
    ...
    def test_apply_completes(self):
        ...
        # archived: true
        state = _read(os.path.join(target, ".codespec.yaml"))
...
''')
    ...
    def test_capture_baseline_writes_correct_hash(self):
        ...
        state = parse_state(os.path.join(self.change_dir(), ".codespec.yaml"))
...
其他设计内容。
''')
        _write(os.path.join(self.change_dir(), "design.md"), u'''# codespec-001-test 设计
...
''')
    ...
    def test_multi_module_design_deltas_touch_only_registered_sections(self):
        ...
        root = td.name
        change = "codespec-004-multidesign"
        _write(os.path.join(root, "codespec", "changes", change, "tasks.md"), TASK_EVIDENCE)
...
系统提供周报功能。
''')
        _write(os.path.join(root, "codespec", "changes", change, "design.md"), u'''# codespec-004-multidesign 设计
...
''')
    ...
    # ---- 测试 23：.codespec.yaml modules 重复 → plan 拒绝且零写入 ----
...
''')
    ...
    def test_parse_state_list_and_errors(self):
        ...
        p = os.path.join(self.change_dir(), ".codespec.yaml")
        _write(p, "codespec: X\nunknown_field: 1\n")
...
系统提供周报功能。
''')
        _write(os.path.join(root, "codespec", "changes", change, "design.md"), u'''# codespec-002-multi 设计
