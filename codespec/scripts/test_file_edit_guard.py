...
class TestDecide(unittest.TestCase):
    ...
    def test_no_active_change_allows_impl(self):
        self.assertTrue(decide("src/app.py", {})[0])

    def test_design_phase_blocks_impl(self):
        ok, reason = decide("src/app.py", {"codespec-001": "design"})
        self.assertFalse(ok)
        self.assertIn("设计", reason)

    def test_open_phase_blocks_impl(self):
        self.assertFalse(decide("src/app.py", {"codespec-001": "open"})[0])

    def test_build_phase_allows_impl(self):
        self.assertTrue(decide("src/app.py", {"codespec-001": "build"})[0])

    def test_verify_phase_allows_impl(self):
        self.assertTrue(decide("src/app.py", {"codespec-001": "verify"})[0])

    def test_own_artifact_allowed_in_design(self):
        self.assertTrue(decide("codespec/changes/codespec-001/spec.md", {"codespec-001": "design"})[0])
        self.assertTrue(decide("codespec/changes/codespec-001/tasks.md", {"codespec-001": "design"})[0])

    def test_any_design_phase_blocks_impl(self):
        # 任一活跃 CodeSpec 处于 open/design，即禁止写实现文件
        self.assertFalse(decide("src/app.py", {"codespec-001": "build", "codespec-002": "design"})[0])
    ...
    def test_archive_phase_allows_impl(self):
        ...
        self.assertTrue(decide("src/app.py", {"codespec-001": "archive"})[0])

    # ---- 归档区默认只读 ----
    def test_archive_dir_readonly(self):
        ok, reason = decide("codespec/changes/archive/2026-08-08-codespec-001/spec.md", {})
        self.assertFalse(ok)
        self.assertIn("归档", reason)
        self.assertFalse(decide("codespec/changes/archive/2026-08-08-codespec-001/.codespec.yaml", {})[0])

    # ---- 未登记 CodeSpec 限制 ----
    def test_unregistered_change_own_yaml_allowed(self):
        # 未登记 CodeSpec 仅允许写自身 .codespec.yaml（初始化状态文件）
        ok, reason = decide("codespec/changes/codespec-NEW/.codespec.yaml", {"codespec-001": "design"})
        self.assertTrue(ok)
        self.assertIn(".codespec.yaml", reason)

    def test_unregistered_change_other_artifact_blocked(self):
        # 未登记 CodeSpec 的其他产物（spec.md/tasks.md）禁止写入
        ok, reason = decide("codespec/changes/codespec-NEW/spec.md", {"codespec-001": "design"})
        self.assertFalse(ok)
        self.assertIn("未登记", reason)
        self.assertFalse(decide("codespec/changes/codespec-NEW/tasks.md", {})[0])
        self.assertFalse(decide("codespec/changes/codespec-NEW/sub/.codespec.yaml", {})[0])
...
class TestHookMain(unittest.TestCase):
    ...
    def _run_hook(self, payload_text, root="C:/repo"):
        ...
        with mock.patch.object(sys, "argv", ["file_edit_guard.py"]), \
                ...
                mock.patch.dict(os.environ, {"CODESPEC_GUARD_ROOT": root}):
            main()
