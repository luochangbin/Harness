...
- **codex-workflow**：面向 ChatGPT 桌面版 Codex 的增强工作流，可选接入 Oracle 设计推理与 Claude Code/OpenCode 执行器。
...
flowchart TB
    ...
    OPEN --> DESIGN["Design<br/>当前 Agent 或 Oracle 辅助推理<br/>生成 design.md、tasks.md"]
...
- **控制权不外包**：外部 Reasoner 提供建议，最终设计由控制 Agent 核对；外部 Executor 完成实现，最终结果由控制 Agent 重新检查和验证。
...
- **不静默降级**：指定的 Reasoner 或 Executor 不可用时明确停止并报告，不擅自切换模型或执行路径。
...
- 当前版本依赖 Superpowers 的 TDD、系统化调试和完成前验证能力。
- Oracle CLI 仅在用户选择它作为 Reasoner 时需要，主要用于plus用户额度不够时利用ChatGPT网页版的5.6 Sol参与设计。
- Claude Code 或 OpenCode 仅在用户选择其作为 Executor 时需要；也可以由当前 Agent 直接执行。
- Oracle 路径可能通过 ChatGPT Web 传输所选上下文。企业项目启用前，应确认源码、日志和配置符合组织的数据安全策略。
